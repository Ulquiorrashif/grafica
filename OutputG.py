...
"")
    ...
    def output(self):
        self.label.setText("Список групп")
        self.label.setAlignment(Qt.AlignCenter)

        con = sqlite3.connect('uni.db')
        cur = con.cursor()
        allList = cur.execute("""SELECT group_title, group_faculty, group_amount FROM groups""").fetchall()
        self.tableWidget.setColumnCount(3)
        self.tableWidget.setRowCount(len(allList))
        self.tableWidget.setHorizontalHeaderLabels(["Группа", "Специальность", "Количество"])
